...
class DevOpsAgentService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.logger.info('DevOps Agent Service initialized successfully');
    } catch (error) {
      this.logger.error('Failed to initialize DevOps Agent Service', error);
      throw error;
    }
  }
  ...
  async start(): Promise<void> {
    try {
      ...
      this.logger.info(`🚀 DevOps Agent Service running on http://${this.config.host}:${this.config.port}`);
    } catch (error) {
      this.logger.error('Failed to start DevOps Agent Service', error);
      throw error;
    }
  }
  ...
  async stop(): Promise<void> {
    try {
      ...
      this.logger.info('DevOps Agent Service stopped gracefully');
    } catch (error) {
      this.logger.error('Error stopping DevOps Agent Service', error);
      throw error;
    }
  }
}
...
if (require.main === module) {
  service.initialize().then(() => {
    ...
    console.error('Failed to start DevOps Agent Service:', error);
    process.exit(1);
  });
}

export { DevOpsAgentService };
